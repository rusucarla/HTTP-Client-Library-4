...
void print_response_message(int status_code, const std::string& status_message, const std::string& additional_info = "", bool show_success = true) {
...
int main(int argc, char* argv[]) {
    ...
    while (true) {
        ...
        if (command == "exit") {
            ...
            print_response_message(response.status_code, response.status_message, "", true);
            ...
            if (session_cookie.empty()) {
                ...
                response = parse_http_response(message, &session_cookie);
                if (!session_cookie.empty() && (response.status_code >= 200 && response.status_code < 300)) {
                    // pentru ca m-am delogat, sterg cookie-ul de sesiune si token-ul JWT
                    session_cookie.clear();
                    jwt_token.clear();
                    print_response_message(response.status_code, response.status_message, "Logged out successfully", true);
                } else {
                    print_response_message(response.status_code, response.status_message, "", false);
                }
            }
            ...
            if (session_cookie.empty() || jwt_token.empty()) {
                ...
                if (title.empty() || author.empty() || genre.empty() || publisher.empty() || page_count.empty() || !std::all_of(page_count.begin(), page_count.end(), ::isdigit)) {
                    std::cout << ERROR_MESSAGE << "Invalid input" << std::endl;
                    continue;
                }
